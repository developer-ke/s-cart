            ...
            <div class="card">
                ...
                <div class="card-header with-border">
                    ...
                    <div class="card-tools not-print">
                        <div class="btn-group float-right" style="margin-right: 0px">
                            <a href="{{ sc_route_admin('admin_mvendor_order.index') }}" class="btn btn-flat btn-default"><i
                ...
                <div class="row" id="order-body">
                        ...
                        <table class="table table-bordered">
                            ...
                            @if (sc_config_admin('customer_country'))
                                <tr>
                                    ...
                                    <td>{{ $country[$order->country] ?? $order->country }}</td>
                                </tr>
                            @endif
                    ...
                    <div class="col-sm-6">
                        <table class="table table-bordered">
                            <tr>
                                <td class="td-title">{{ sc_language_render('order.status') }}:</td>
                                <td>{{ $statusOrder[$order->status] ?? '' }}</td>
                            </tr>
                            <tr>
                                <td>{{ sc_language_render('order.shipping_status') }}:</td>
                                <td><a href="#" class="updateStatus" data-name="shipping_status_store"
                                        data-type="select" data-source ="{{ json_encode($statusShipping) }}"
                                        data-pk="{{ $order->id }}" data-value="{!! $order->shipping_status_store !!}"
                                        data-url="{{ sc_route('admin_mvendor_order.update') }}"
                                        data-title="{{ sc_language_render('order.shipping_status') }}">{{ $statusShipping[$order->shipping_status_store] ?? '' }}</a>
                                </td>
                            </tr>
                            <tr>
                        ...
                        <table class="table table-bordered">
                            <tr>
                                <td><i class="far fa-money-bill-alt nav-icon"></i>
                                    {{ sc_language_render('order.currency') }}:</td>
                                <td>{{ $order->currency }}</td>
                            </tr>
                            <tr>
                                <td></i> {{ sc_language_render('order.domain') }}:</td>
                                <td>{{ $order->domain }}</td>
                            </tr>
                            <tr>
                                            ...
                                            <tr>
                                                <td>{{ $item->name }}
                                                    @php
                                                        ...
                                                        if (
                                                            $item->attribute &&
                                                            is_array(json_decode($item->attribute, true))
                                                        ) {
                                ...
                                @foreach ($dataTotal as $element)
                                    @if ($element['code'] == 'subtotal')
                                        <tr>
                                            ...
                                            <td style="text-align:right" class="data-{{ $element['code'] }}">
                                                {{ sc_currency_format($element['value']) }}</td>
                                        </tr>
                                    @endif
                                    @if ($element['code'] == 'tax')
                                        <tr>
