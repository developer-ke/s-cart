                ...
                <div class="card-header with-border">
                    <div class="card-tools">
                        <div class="menu-right">
                            <a href="{{ sc_route_admin('admin_MultiVendor.create') }}" class="btn btn-success btn-flat btn-md"
                                title="New" id="button_create_new">
                                <i class="fa fa-plus" title="{{ sc_language_render('store.admin.add_new') }}"></i> <span
                                    class="fab fa-shopify"></span>
                            </a>
                        </div>
                    </div>
                </div>
                            ...
                            <tbody>
                                @foreach ($stories as $key => $store)
                                    <tr>
                                        ...
                                        <td>
                                            &nbsp; <a
                                                title="{{ sc_language_render($pathPlugin . '::lang.admin.store_shop') }}"
                                                target=_new
                                                href="{{ sc_route_admin('MultiVendor.detail', ['code' => $store->code]) }}"><span
                                                    class="fab fa-shopify"></span></a> &nbsp;
                                        </td>
                                        <td>
                                            @if ($key != SC_ID_ROOT)
                                                <input data-store="{{ $store->id }}" name="status"
                                                    class="check-data-config-store" type="checkbox"
                                                    {{ $store->status == '1' ? 'checked' : '' }}>
                                            @endif
                                        </td>
                                        <td>
                                            @if ($key != SC_ID_ROOT)
                                                <span onclick="deleteItem('{{ $store->id }}');"
                                                    title="{{ sc_language_render($pathPlugin . '::lang.admin.store_remove') }}"
                                                    class="btn btn-flat btn-danger">
                                                    <i class="fas fa-trash-alt"></i>
                                                </span>
                                            @endif

                                            <a href="{{ sc_route_admin('admin_MultiVendor.config', ['id' => $store->id]) }}"
                                                title="{{ sc_language_render($pathPlugin . '::lang.admin.store_config') }}">
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="6" style="font-size: 12px">
                                        <span
                                            style="color:red">*{{ sc_language_render($pathPlugin . '::lang.admin.store_open') }}</span>
                                        : {{ sc_language_render($pathPlugin . '::lang.admin.store_open_help') }}<br>
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
                            ...
                            success: function(data) {
                                ...
                                if (data.error == 1) {
                                    alertMsg('error', data.msg,
                                        '{{ sc_language_render('admin.warning') }}');
