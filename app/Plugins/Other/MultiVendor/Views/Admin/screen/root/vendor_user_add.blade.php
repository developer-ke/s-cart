                ...
                <div class="card-header with-border">
                    ...
                    <div class="card-tools">
                        <div class="btn-group float-right mr_5">
                            <a href="{{ sc_route_admin('admin_MultiVendorUser.index') }}" class="btn  btn-flat btn-default"
                ...
                <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal"
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-sm-8">
                                <select class="form-control country" style="width: 100%;" name="country">
                                    ...
                                    @foreach ($countries as $k => $v)
                                        <option value="{{ $k }}" {{ $country == $k ? 'selected' : '' }}>
                                            {{ $v }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group  row {{ $errors->has('password') ? ' text-red' : '' }}">
                            <label for="password"
                                class="col-sm-2  col-form-label">{{ sc_language_render('multi_vendor.password') }}</label>
                            <div class="col-sm-8">
                                <div class="input-group">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
                        ...
                        <div class="form-group row {{ $errors->has('store_id') ? ' text-red' : '' }}">
                            ...
                            <div class="col-sm-8">
                                <select class="form-control store_id select2"
                                    data-placeholder="{{ sc_language_render('admin.select_store') }}"
                                    style="width: 100%;" name="store_id">
                                    <option value=""></option>
                                    @foreach (sc_get_list_code_store() as $k => $v)
                                        @if ($k != 1)
                                            <option value="{{ $k }}" {{ $store_id == $k ? 'selected' : '' }}>
                                                {{ $v }}
                                            </option>
                                        @endif
                                    @endforeach
                        ...
                        <div class="form-group  row">
                            <label for="status"
                                class="col-sm-2  col-form-label">{{ sc_language_render('multi_vendor.status') }}</label>
                            <div class="col-sm-8">
                                <input class="checkbox" type="checkbox" name="status"
                                    {{ old('status', empty($vendor['status']) ? 0 : 1) ? 'checked' : '' }}>
